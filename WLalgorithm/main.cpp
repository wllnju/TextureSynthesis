...
int main( int argc, char** argv )
{
	...
	for (int i = 0; i < N; i++) {
		output[i] = imagename;
		output[i].insert(output[i].find("."), "_textured_" + convertInt(window_size[i]));		
		...
		cout << output[i] << "\nsynthesized by exhaustive_search method using window " << window_size[i] << " X " << window_size[i] << endl;
